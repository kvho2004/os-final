...
       Smaller = more responsive to cat arrival; larger = less CPU overhead.  */
    ...
    while (1) {
        ...
        // Wait for no cats to be present
        ...
        // Feed in short bursts, fleeing if a cat arrives
        int fled = 0;
        while (!fled) {
            // Sleep one burst
            usleep(BURST_MS * 1000);

            // Check if a cat has arrived or not 
            pthread_mutex_lock(&state->cat_count_mutex);
            if (state->active_cats > 0) {
                fled = 1;
